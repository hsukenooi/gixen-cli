...
- Exception hierarchy: `GixenError` is the base; `GixenLoginError`, `GixenSessionExpiredError`, `GixenItemError` (has `.code` and `.message`), `GixenSnipeNotFoundError`, `GixenParseError`, `GixenAddNotConfirmedError` all inherit from it. `GixenAddNotConfirmedError` is raised by `add_snipe` when the POST returned no error but the snipe never appeared in the list (verify miss), or when the verify itself failed (parse/network) and double-POSTing would risk a duplicate.
